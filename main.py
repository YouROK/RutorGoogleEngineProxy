...
class MainHandler(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        url = self.request.scheme + '://' + subdomain + self.host_name + path_qs
        logging.debug(url)
        ...
        for name, value in self.request.headers.iteritems():
            if not name.startswith('X-'):
                headers[name] = value.replace(self.proxy_host_name, self.host_name)
        ...
        for header_line in result.header_msg.headers:
            ...
            if 'content-type' == name_l:
                ...
                value = value.replace(self.host_name, self.proxy_host_name)
    ...
    def modify_content(self, content, mode=None):
        ...
        regexp += re.escape(self.host_name)
